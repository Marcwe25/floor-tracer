...
class InternalBuilder:
    ...
    def build_interior(self):
        ...
        for x_keys in index.keys():
            for y_keys in index[x_keys]:
                ...
                connected_points = shape.get_facing_connected_point(point)
                connected_points = {shape.get_point(x) for x in connected_points}
                connected_points = {x for x in connected_points if x != point}
    ...
    def get_sub_paths(self, base_point):
        ...
        for second_point in all_points[base_point]:
            ...
            if second_point not in path_s_second_point[base_point]:
                ...
                while len(all_points[next_point]) < 3 and next_point != base_point:
                    # print("just 2",next_point)
                    new_path.append(next_point)

                    point_set = all_points[next_point].difference(set(new_path).difference({base_point}))

                    if base_point in point_set and len(point_set) > 1:
                        next_point = self.filter_point_clockwise(new_path[-2], next_point, point_set)
    ...
    def filter_path_to_go(self, current_path, path_set):
        ...
        #print("cur pa", current_path)
        m_path = [path for path in path_set if len(set(path).intersection(current_path)) == 1]

        m_path = [(path, get_angle(current_path[-1], current_path[-2], path[1])) for path in m_path if
                  path != current_path]
        m_path = [(path, angle) for path, angle in m_path if 359.5 > angle > 0.5]

        if len(m_path) == 0:
            #print("filter return none")
            return None
        m = max(m_path, key=itemgetter(1))[0]
        #print("filter return", m)
        return m

    def filter_point_clockwise(self, p, c, potential_points):
