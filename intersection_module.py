...
from corrector_module import CorrectorSimple
            ...
            if intersection is not None:
                if intersection.z == 100:
                    ...
                    if intersection == point_a1:
                        #print("moving a1 ",point_a1, " to ", intersection)
                        shape.move_cluster(point_a1, intersection.get_position())
                    elif intersection == point_a2:
                        #print("moving a2 ",point_a2, " to ", intersection)
                        shape.move_cluster(point_a2, intersection.get_position())
                    else:
                        #print("adding intersection 1 to ",point_pair[0][0], " at ", intersection)
                        intersection_dict[point_pair[0][0]].add(intersection)

                    if intersection == point_b1:
                        #print("moving b1 ",point_b1, " to ", intersection)
                        shape.move_cluster(point_b1, intersection.get_position())
                    elif intersection == point_b2:
                        #print("moving b2 ",point_b2, " to ", intersection)
                        shape.move_cluster(point_b2, intersection.get_position())
                    else:
                        #print("adding intersection 2 to ",point_pair[1][0], " at ", intersection)
                ...
                if intersection.z == 100:
                    ...
                    if intersection != point_a1 and intersection != point_a2:
                        if closest_point_on_line_param(point_a1, point_a2, intersection) > 1:
                            if point_between_points2(point_b1, point_b2, intersection, error_thresold):
